...
def isospin_two_spin_contract(q1,q2,q3,corr,spin):
    src_weights = np.zeros([2],dtype=np.complex128)
    src_weights[0] = 1./np.sqrt(2)
    src_weights[1] = -1./np.sqrt(2)
    snk_weights = np.zeros([4],dtype=np.complex128)
    ...
    snk_weights[2] =  1./np.sqrt(2)
    snk_weights[3] = -1./np.sqrt(2)
    if corr == 'omega_m' or corr == 'omega_m_np':
        coeff = 4/3
    else:
        coeff = 1
...
def isospin_three_spin_contract(q1,q2,q3,corr,spin):
    src_weights = np.zeros([2],dtype=np.complex128)
    src_weights[0] = 1./np.sqrt(2)
    src_weights[1] = -1./np.sqrt(2)
    snk_weights = np.zeros([4],dtype=np.complex128)
...
def isospin_three_half_spin_contract(q1,q2,q3,corr,spin):
    ...
    if corr == 'delta_z' or corr == 'delta_m' or corr == 'delta_p' or corr == 'delta_pp':
        ...
        if spin =='dndn':
            ...
            print('unrecognized spin - aborting',spin)
            sys.exit(-1)
    elif corr == 'delta_z_np' or corr == 'delta_m_np' or corr == 'delta_p_np' or corr == 'delta_pp_np':
        if spin == 'up':
            src_spins[0,0] = 2; src_spins[0,1] = 2; src_spins[0,2] = 3;
            src_spins[1,0] = 2; src_spins[1,1] = 3; src_spins[1,2] = 2;
...
def sigma_star_spin_contract(q1,q2,q3,corr,spin):
    src_weights = np.zeros([4],dtype=np.complex128)
    src_weights[0] = 1.
    src_weights[1] = -1.
    src_weights[2] = -1.
    ...
    if corr == 'lambda_to_sigma_star':
        if spin == 'up':
            ...
            print('unrecognized spin - aborting',spin)
            sys.exit(-1)
    elif corr == 'lambda_to_sigma_star_np':
        if spin == 'up':
            src_spins[0,0] = 2; src_spins[0,1] = 2; src_spins[0,2] = 3;
            src_spins[1,0] = 2; src_spins[1,1] = 3; src_spins[1,2] = 2;
            src_spins[2,0] = 2; src_spins[2,1] = 2; src_spins[2,2] = 3;
